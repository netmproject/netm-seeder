...
NetM-seeder is a crawler for the NetM network, which exposes a list
...
5. netm-seeder takes this request and returns a set of IPs back to the client.

So in short: Set an NS record on dnssseed.example.com pointing to whereever the 
netm-seeder is running. Make sure the address you put in there 
(vps.example.com) points to this server. Run netm-seeder with the above parameters.
